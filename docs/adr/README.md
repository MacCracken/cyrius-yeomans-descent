...
- [`0006-persistence-shape.md`](0006-persistence-shape.md) — Accepted — per-player signed `data/players/<c>/<name>.cyml` (sharded since 1.7.2) (atomic `.tmp`+rename) for state; libro hash-chain audit (`data/audit.libro`) for security events. libro is the crash-safe primitive; the t-ron MCP repo is not used.
...
- **`0008` — RESERVED for M14** (the 2.x surface freeze that supersedes 0007). Not yet written; referenced by number in the roadmap, in `state.md`, and in the shipped CHANGELOG. Do not take this number.
- [`0009-audit-log-rotation.md`](0009-audit-log-rotation.md) — Accepted — audit-log rotation by *seal and continue*: rename the live file to a numbered segment and keep appending, so the streaming chain's carried head hash records the boundary itself. No on-disk format change and no libro release; amends 0006's audit-chain half. Decision made in 1.7.1, mechanism in 1.7.2.
